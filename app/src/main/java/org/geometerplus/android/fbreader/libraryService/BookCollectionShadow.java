...
/**
 * author:WuZepeng </br>
 * time:2017-09-25 14:49 </br>
 * desc:一个用来关联用来读取或者修改与书相关的信息的Service的类，
 * 通过LibraryInterface访问Service的进程中的数据 </br>
 */
...
//			Logger.e("wzp used in File:%s%nClass:%s%nMethod:%s%nLine:%d%n",element.getFileName(),element.getClassName(),element.getMethodName(),element.getLineNumber());
//		}
		if (myInterface != null && myContext == context) {
			...
			);//在第一次调用该方法时，建立LibraryService和FBReader之间的通信
			if (result) {
				myContext = context;
			}
			return result;
	...
	public void onServiceConnected(ComponentName name, IBinder service) {
		...
		if (myContext != null) {
			myContext.registerReceiver(myReceiver, new IntentFilter(FBReaderIntents.Event.LIBRARY_BOOK),FBReaderIntents.Event.CUSTOM_BROADCAST_PSERMISSION,null);
			myContext.registerReceiver(myReceiver, new IntentFilter(FBReaderIntents.Event.LIBRARY_BUILD),FBReaderIntents.Event.CUSTOM_BROADCAST_PSERMISSION,null);
		}
	}
