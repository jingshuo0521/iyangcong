...
public final class SQLiteBooksDatabase extends BooksDatabase {
    ...
    private String getLanguagetType(int type) {
        switch (type) {
            case 1:
                return "'zh'";
            case 2:
    ...
    /**
     * 根据书的标题和语言版本查找书在Books表中的id，如果查询不到的话就返回-1;
     *
     * @param languageType
     * @param bookName
     * @return
    ...
    public long getBookIdByTitleAndLanguage(int languageType, String bookName) {
        long bookId = -1;
        Cursor cursor = myDatabase.rawQuery("SELECT book_id,file_id,encoding FROM Books WHERE language = " + getLanguagetType(languageType) + " AND title = '" + bookName + "'", null);
        while (cursor.moveToNext()) {
            bookId = cursor.getLong(0);
            break;
        }
        Logger.i("wzp bookId:" + bookId);
        return bookId;
    }

    public boolean updateBookMarker(Bookmark bookmark) {

//        myDatabase.query("Bookmarks",new String[]{"bookmark_id"},"bookmark_text =?",new String[]{bookmark.getText()},null,null,null);
        final Cursor cursor = myDatabase.rawQuery("SELECT bookmark_text FROM Bookmarks WHERE bookmark_text=?", new String[]{bookmark.getText()});
        if (cursor.moveToNext()) {
            Logger.i(cursor.getString(0));
//            myDatabase.execSQL(
//                    "UPDATE Bookmarks SET `original_text` = '" + bookmark.getOriginalText() + "' WHERE `bookmark_text` = '" + bookmark.getText() + "'"
//            );
            myDatabase.execSQL("UPDATE Bookmarks SET original_text = ? WHERE bookmark_text = ?",new String[]{bookmark.getOriginalText(),bookmark.getText()});
            return true;
        } else {
            return false;
        }
...
//    public Bookmark deleteBookmark(){
//        Cursor cursor = myDatabase.rawQuery("SELECT * FROM Bookmarks ",null);
//        if (cursor.moveToFirst()){
//            do {
//                myDatabase.execSQL(BookmarkF);
//            }
//        }
//    }

//    public Bookmark deletBookMarker(List<BookNoteBean> list) {
//
//        myDatabase.execSQL("DELETE FROM Bookmarks " ,null);
//        final Cursor cursor = myDatabase.rawQuery("SELECT bookmark_text FROM Bookmarks WHERE bookmark_text=?", new String[]{bookmark.getText()});
//        if (cursor.moveToNext()) {
//            Logger.i(cursor.getString(0));
//            myDatabase.execSQL(
...
//            );
//
//            Bookmark bookMark = myDatabase.execSQL("UPDATE Bookmarks SET original_text = ? WHERE bookmark_text = ?",new String[]{bookmark.getOriginalText(),bookmark.getText()});
//
//        }
//        return bookMark;
//    }
    ...
    public List<Bookmark> getBookMarks(BookmarkQuery query) {
