			...
			for (ZLTextParagraph.EntryIterator it = myParagraph.iterator(); it.next(); ) {
				switch (it.getType()) {
					case ZLTextParagraph.Entry.TEXT:
						//对文本信息调用Processor类的processTextEntry方法，
						// 将ZLTextWord类加入ZLTextParagraphCursor类的myElements属性指向的ArrayList;
						processTextEntry(it.getTextData(), it.getTextOffset(), it.getTextLength(), hyperlink);
						break;
					case ZLTextParagraph.Entry.CONTROL:
						//对于标签信息
						//则直接将ZLTextControlElement类加入
						//ZLTextParagraphCursor类的myElements属性指向的ArrayList;
						if (hyperlink != null) {
					...
					case ZLTextParagraph.Entry.AUDIO:
						elements.add(new ZLTextAudioElement(it.getAudioEntry().sources()));
						break;
		...
		private void processTextEntry(final char[] data, final int offset, final int length, ZLTextHyperlink hyperlink) {
			if (length != 0) {
				...
				for (int index = 0; index < length; ++index) {
					...
					//判断ch元素是否是空白字符串,并且不是一个不换行空格；
					if (Character.isWhitespace(ch)) {
						if (index > 0 && spaceState == NO_SPACE) {
							addWord(data, offset + wordStart, index - wordStart, myOffset + wordStart, hyperlink);
						}
//						Log.e("PapaText isWhiteSpace",":"+new String(data, offset + wordStart, index - wordStart));
						spaceState = SPACE;
					} else if (Character.isSpaceChar(ch)) {
						//判断字符是否是Unicode类型的空白字符，空白字符指的是以下三种类型：
						// /SPACE_SEPARATOR、LINE_SEPARATOR、PARAGRAPH_SEPARATOR
						if (index > 0 && spaceState == NO_SPACE) {
							addWord(data, offset + wordStart, index - wordStart, myOffset + wordStart, hyperlink);
						}
//						Log.e("PapaText isSpaceChar",":"+new String(data, offset + wordStart, index - wordStart));
						...
						switch (spaceState) {
							...
							case NON_BREAKABLE_SPACE:
								wordStart = index;
								break;
							//char元素不是空格的情况（也就是正常的文本）
							case NO_SPACE:
								if (index > 0 &&
										(breaks[index - 1] != LineBreaker.NOBREAK || StringUtils.checkPunctuation(ch) )&&
										previousChar != '-' &&
										index != wordStart) {
									//BUG #2482
									//2019/5/30 修复成功
									//添加了判断逻辑  让标点符号也作为一个ZLWord对象  实现了文字选取时  标点符号不与前一个文字或单词粘连
//									Logger.e("Wzp NoSpace addWord,offset:%d wordStart:%d index:%d myOffset:%d",offset,wordStart,index,myOffset);
									addWord(data, offset + wordStart, index - wordStart, myOffset + wordStart, hyperlink);
//									Log.e("PapaText NO_SPACE",":"+new String(data, offset + wordStart, index - wordStart));
									wordStart = index;
								}
								break;
						}
...
//						Log.e("PapaText NO_SPACE 1",":"+new String(data, offset + wordStart, length - wordStart));
						break;
				}
				myOffset += length;
			}
	...
	public final int Index;//这个字段只在构造器中写值，别的地方都没有写值，都是取值。
	...
	/**
	 * author:WuZepeng </br>
	 * time:2018-05-21 14:33 </br>
	 * desc:判断段落是否为没有element的空段落 </br>
	 */
	public boolean isEmpty(){
		return  myElements!=null&&myElements.size() == 0;
	}
