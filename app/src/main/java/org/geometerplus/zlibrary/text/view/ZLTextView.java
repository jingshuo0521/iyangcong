...
public abstract class ZLTextView extends ZLTextViewBase {
	...
	private int myScrollingMode;//只有调用了turnPage方法的时候值才会变化
	private int myOverlappingValue;//只有调用了turnPage方法的时候值才会变化
	...
	public synchronized void paint(ZLPaintContext context, PageIndex pageIndex) {
		...
		for (ZLTextHighlighting h : hilites) {
			...
			final ZLColor bgColor = h.getBackgroundColor();//new ZLColor(240,105,50);
			if (bgColor != null) {
				context.setFillColor(bgColor, 128);
				mode |= Hull.DrawMode.Fill;
//				context.setLineColor(bgColor);
//				mode |= Hull.DrawMode.Outline;
	...
	/**
	 * author:WuZepeng </br>
	 * time:2017-09-28 17:54 </br>
	 * desc:根据ZLTextWord类中的文本信息与ZLTextElementArea类中的位置信息
	 * 调用Canvas类的drawText方法对Bitmap类进行操作，最终将字一个一个“画”到“画布”上去 </br>
	 */
	private void drawTextLine(ZLTextPage page, List<ZLTextHighlighting> hilites, ZLTextLineInfo info, int from, int to) {
		...
		for (int wordIndex = info.RealStartElementIndex; wordIndex != endElementIndex && index < to; ++wordIndex, charIndex = 0) {
			...
			if (element == area.Element) {
				...
				if (element instanceof ZLTextWord) {
				...
				} else if (element instanceof ZLEndParagraph) {
					context.drawString(areaX, areaY,"####");
					...
					// TODO: draw
//					LinearLayout ly = new LinearLayout(fbReaderApp.getmContext());
//					LinearLayout.LayoutParams lp1 = new LinearLayout.LayoutParams(
//							LinearLayout.LayoutParams.WRAP_CONTENT, LinearLayout.LayoutParams.WRAP_CONTENT);
//					final LayoutInflater inflater = LayoutInflater.from(fbReaderApp.getmContext());
//					View ly = inflater.inflate(R.layout.video, null, false);
//					View layoutView = View.inflate(fbReaderApp.getmContext() , R.layout.video , null) ;
//					Uri uri = Uri.parse("https://www.zhangxinxu.com/study/media/cat2.mp4");
//					VideoView videoView = (VideoView)layoutView.findViewById(R.id.view);
//					videoView.setMediaController(new MediaController(fbReaderApp.getmContext()));
...
//					videoView.requestFocus();
	...
	/**
	 * author:WuZepeng </br>
	 * time:2017-09-28 17:53 </br>
	 * desc:进一步获得每一行中的每一个字的位置信息与样式信息，
	 * 代码会用这些信息新建一个ZLTextElementArea类，
	 * 并将所有新建的ZLTextElementArea类加入到ZLTextPage类的TextElementMap属性 </br>
	 */
	...
	/**
	 * author:WuZepeng </br>
	 * time:2017-09-28 17:38 </br>
	 * desc:preparePaintInfo方法从定位指定段落后得到的ZLTextPage类中
	 * 取出一个一个取出代表段落中每个字的ZLTextElement子类，
	 * 计算出屏幕上的每一行对应着哪些字 </br>
	 */
...
*/
	...
	protected ZLTextHighlighting findHighlighting(int x, int y, int maxDistance) {
		...
		List<ZLTextHighlighting> result = new ArrayList<>();
		synchronized (myHighlightings) {
			for (ZLTextHighlighting h : myHighlightings) {
//				result.add(h);
...
//		}
	...
	private Comparator<ZLTextHighlighting> comparator = new Comparator<ZLTextHighlighting>() {
		@Override
		public int compare(ZLTextHighlighting t1, ZLTextHighlighting t2) {
			//如果t1长度比t2短 那就将t1排在t2前面
			int length1 = t1.getEndPosition().getElementIndex() - t1.getStartPosition().getElementIndex();
			int length2 = t2.getEndPosition().getElementIndex() - t2.getStartPosition().getElementIndex();
//			int length1 = t1.getStartPosition().getElementIndex().compareTo(t1.getEndPosition().getElementIndex());
//			int lenght2 = t2.getStartPosition().compareTo(t2.getEndPosition());
			int result = length1 - length2;
//			int result = t1.getStartPosition().compareTo(t2.getStartPosition()) * t1.getEndPosition().compareTo(t2.getEndPosition());
			return result == 0?t1.getStartPosition().compareTo(t2.getStartPosition()):result;

//			if(t1.getStartPosition().compareTo(t2.getStartPosition()) > 0&& t1.getEndPosition().compareTo(t2.getEndPosition()) <0){
//				return 1;
//			}else if(t1.getStartPosition().compareTo(t2.getStartPosition()) < 0&& t1.getEndPosition().compareTo(t2.getEndPosition()) >0){
//				return -1;
//			}else if(t1.getStartPosition().compareTo(t2.getStartPosition()) > 0&& t1.getEndPosition().compareTo(t2.getEndPosition()) >0){
//				//如果t1和t2有一部分相交 将t1排在t2前面
//				return 1;
//			}else if(t1.getStartPosition().compareTo(t2.getStartPosition()) < 0&& t1.getEndPosition().compareTo(t2.getEndPosition()) <0 ){
//				return -1;
//			}
