...
public final class ZLTextPlainModel implements ZLTextModel, ZLTextStyleEntry.Feature {
	...
	private int[] myStartEntryIndices;//记录了每个段落具体在CachedCharStorage类内部的哪一个char数组里面；
	private int[] myStartEntryOffsets;//记录了每个段落从CachedCharStorage类内部char数组的哪个位置开始；
	private int[] myParagraphLengths;//记录每个段落在CachedCharStorage类内部char数组中占据多少长度
	private int[] myTextSizes;
	private byte[] myParagraphKinds;
	private byte[]	myParagraphTags;

	private int myParagraphsNumber;

	private final CachedCharStorage myStorage;//指向的CachedCharStorage类内部的char数组则是实际存储文本信息与标签信息的地方
	...
	final class EntryIteratorImpl implements ZLTextParagraph.EntryIterator {
		...
		//判断当前段落之后是否还有内容，如果有内容，对内容进行解析.
		// 由于内容的类型有很多种，所有解析的时候分多钟情况
		public boolean next() {
			...
			++dataOffset;
			switch (type) {
				//遇到常量ZLTextParagraph.Entry.TEXT，按照文本信息处理
				case ZLTextParagraph.Entry.TEXT:
				{
					...
					//向前跳char数组中涉及这段文本信息的部分；
					dataOffset += textLength;
					break;
				}
				//遇到ZLTextParagraph.Entry.CONTROL,按照标签信息处理
	...
	public final ZLTextParagraph getParagraph(int index) {
		...
		final byte tag = myParagraphTags[index];
		return (kind == ZLTextParagraph.Kind.TEXT_PARAGRAPH) ?
			new ZLTextParagraphImpl(this, index,tag):
