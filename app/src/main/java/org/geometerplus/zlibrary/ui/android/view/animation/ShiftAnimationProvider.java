...
public final class ShiftAnimationProvider extends SimpleAnimationProvider {
	...
	protected void drawInternal(Canvas canvas) {
		if (myDirection.IsHorizontal) {
			...
			drawBitmapTo(canvas, dX > 0 ? dX - myWidth : dX + myWidth, 80, myPaint);
			...
			drawBitmapTo(canvas, 0, dY > 0 ? dY - myHeight : dY + myHeight + 80, myPaint);
	...
	protected void drawHeadBitmapInternal(Canvas canvas, Bitmap headBitmap, int voffset) {
		...
		if (myDirection.IsHorizontal) {
			final int dX = myEndX - myStartX;
			if (dX > 0 && dX < myWidth) {
				canvas.drawLine(dX, voffset, dX, voffset + headBitmap.getHeight(), myPaint);
			} else if (dX < 0 && dX > -myWidth) {
				canvas.drawLine(dX + myWidth, voffset, dX + myWidth, voffset + headBitmap.getHeight(), myPaint);
			}
		}
	}
