...
public final class SlideAnimationProvider extends SimpleAnimationProvider {
	...
	protected void drawInternal(Canvas canvas) {
		if (myDirection.IsHorizontal) {
			...
			drawBitmapTo(canvas, 0, 80, myDarkPaint);
			drawBitmapFrom(canvas, dX, 80, myPaint);
			drawShadowVertical(canvas, 0, myHeight, dX);
		} else {
			final int dY = myEndY - myStartY;
			setDarkFilter(dY, myHeight);
			drawBitmapTo(canvas, 0, 80, myDarkPaint);
			drawBitmapFrom(canvas, 0, dY + 80, myPaint);
	...
	@Override
	protected void drawHeadBitmapInternal(Canvas canvas, Bitmap headBitmap, int voffset) {
		if (myDirection.IsHorizontal) {
			final int dX = myEndX - myStartX;
			setDarkFilter(dX, myWidth);
			final int h = headBitmap.getHeight();
			if (dX > 0) {
				drawBitmapInternal(canvas, headBitmap, 0, dX, h, voffset, myDarkPaint);
				drawBitmapInternal(canvas, headBitmap, dX, myWidth, h, voffset, myPaint);
			} else {
				drawBitmapInternal(canvas, headBitmap, myWidth + dX, myWidth, h, voffset, myDarkPaint);
				drawBitmapInternal(canvas, headBitmap, 0, myWidth + dX, h, voffset, myPaint);
			}
			drawShadowVertical(canvas, voffset, voffset + h, dX);
		} else {
			canvas.drawBitmap(headBitmap, 0, voffset, myPaint);
		}
	}
