...
/**
 * author:WuZepeng </br>
 * time:2017-09-21 15:51 </br>
 * desc:关于通过数据库获取书的相关信息的工具类 </br>
 */
public class BookCollection extends AbstractBookCollection<DbBook> {
	...
	private DbBook getBookByFile(ZLFile bookFile, final FormatPlugin plugin) {
		...
		fileInfos.save();
//		Logger.e("wzp if book == null?:"+(book==null));
		try {
			if (book == null) {
		...
		Logger.e("wzp bookTitle:%s%n",book.getTitle());
		saveBook(book);
		return book;
	}

	public long getBookIdByTitleAndLanguage(int languageType,String bookName){
		return myDatabase.getBookIdByTitleAndLanguage(languageType,bookName);
	}
	@Override
	public DbBook getBookById(long id) {
		DbBook book = myBooksById.get(id);
		if (book != null) {
//			Logger.e("wzp bookTitle:"+book.getTitle());
			return book;
		}
	...
	private void processFilesQueue() {
		synchronized (myFilesToRescan) {
			...
			for (ZLFile file : collectPhysicalFiles(myFilesToRescan)) {
				...
				Logger.e("wzp saveBook:%s%n",book.getTitle());
				if (book != null) {
					saveBook(book);
					getHash(book, false);
				}
	...
	private void build() {
		...
		Log.e("wzp savedBooksByFileId:",savedBooksByFileId.values().size()+"");
		...
		for (DbBook book : savedBooksByFileId.values()) {
			...
			if (book.File.exists()) {
				...
				if (!fileInfos.check(file, true)) {
					try {
						Logger.e("wzp shortName:%s%n",book.File.getShortName());
						BookUtil.readMetainfo(book, PluginCollection);
						saveBook(book);
					} catch (BookReadingException e) {
						doAdd = false;
		...
		Logger.e("wzp orphanedBooksByFileId %d%n:",orphanedBooksByFileId.values().size());
	...
	private void collectBooks(
		...
		try {
			...
			Logger.e("wzp orphanedBooksByFileId:%s%n",book.getTitle());
			if (book != null) {
				if (doReadMetaInfo) {
					BookUtil.readMetainfo(book, PluginCollection);
				}
	...
	public void saveBookmark(Bookmark bookmark) {
		if (bookmark != null) {
			...
			if(bookmark.getText().contains("$%^&*")){
				isOver = true;
				bookmark.setText(bookmark.getText().replace("$%^&*",""));
			}
			...
			if (bookmark.IsVisible) {
				...
				if (book != null) {
					...
					if(isOver){
						...
						Logger.e("fireBookEvent:%s%n",  bookmark.getText()+ "");
					}
				}
			}
		}
