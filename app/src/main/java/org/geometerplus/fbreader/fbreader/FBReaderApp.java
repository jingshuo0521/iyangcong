...
public final class FBReaderApp extends ZLApplication {
    ...
    public FBReaderApp(org.geometerplus.zlibrary.core.util.SystemInfo systemInfo, final IBookCollection<Book> collection,Context context) {
        super(systemInfo);
        mContext = context;
...
//		addAction(ActionCode.DECREASE_FONT, new ChangeFontSizeAction(this, -2));
    ...
    public void refreshBookMarkers(Book book){
    ...
    public void openBook(Book book, final Bookmark bookmark, Runnable postAction, Notifier notifier, int paragraphIndex) {
        if (bookmark != null) {
            openBook(book, bookmark, postAction, notifier);
        } else {
            posZhEn = new ZLTextFixedPosition(paragraphIndex, 0, 0);
            openBook(book, null, postAction, notifier);
        }
    }
    ...
    public Bookmark createNewBookmark(int paragraph, boolean isVisible) {
        final FBView fbView = getTextView();
        final TextSnippet snippet = new EmptyTextSnippet(new ZLTextFixedPosition(paragraph, 0, 0));
        if (snippet == null) {
            return null;
        }
    ...
    private void setBookmarkHighlightings(final ZLTextView view, final String modelId) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                view.removeHighlightings(BookmarkHighlighting.class);
                SQLiteBooksDatabase database = new SQLiteBooksDatabase(mContext);
                for (BookmarkQuery query = new BookmarkQuery(Model.Book, 20); ; query = query.next()) {
                    final List<Bookmark> bookmarks = database.getBookMarks(query);
                    ...
                    for (Bookmark b : bookmarks) {
                        ...
                        Logger.e("bookmark.b:%s%n", b.getText() + "");
...
//        }
        ...
        if (plugin instanceof ExternalFormatPlugin) {
            ...
            if (bookmark != null) {
                ...
                if (posZhEn != null) {
                    bm = new Bookmark(Collection, book, "", new EmptyTextSnippet(posZhEn), false);
                    posZhEn = null;
                } else {
    ...
    private void gotoBookmark(Bookmark bookmark, boolean exactly) {
        try {
            int maxParargaph = getTextView().getStartCursor().getParagraphCursor().Model.getParagraphsNumber();
            if(bookmark.getParagraphIndex()>maxParargaph){
				bookmark = new Bookmark(bookmark,maxParargaph,0,0,maxParargaph,0,0);
			}
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void savePosition(Book book, int paragrahId) {
        synchronized (mySaverThread) {
            if (!mySaverThread.isAlive()) {
                mySaverThread.start();
            }
            mySaverThread.add(new PositionSaver(book, new ZLTextFixedPosition(paragrahId, 0, 0), null));
        }
    }
    ...
    public Bookmark getBookmark(int maxChars, boolean visible, String text) {
        ...
        bookmark.setMyOriginalText(text);
        bookmark.setStyleId(0);
...
//        Collection.saveBookmark(bookmark);
    ...
    public Bookmark addBookmark(int maxChars, boolean visible) {
        final FBView view = getTextView();
        final ZLTextWordCursor cursor = view.getStartCursor();
    ...
    public void addBookmark(Bookmark myBookmark) {
        final FBView view = getTextView();

        Collection.saveBookmark(myBookmark);
        view.clearSelection();
    }
