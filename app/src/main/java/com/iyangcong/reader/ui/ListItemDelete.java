...
import com.iyangcong.reader.activity.AllWordActivity;
import com.iyangcong.reader.activity.ReciteAlreadyKnowActivity;
import com.iyangcong.reader.activity.ReciteMyFavoriteActivity;
...
public class ListItemDelete extends LinearLayout {
	...
	public void reSet(){
		scrollTo(0, 0);
		if(activity instanceof AllWordActivity){
			AllWordActivity.itemDelete = null;
		}else if (activity instanceof ReciteAlreadyKnowActivity){
			ReciteAlreadyKnowActivity.itemDelete = null;
		}else if (activity instanceof ReciteMyFavoriteActivity){
			ReciteMyFavoriteActivity.itemDelete = null;
		}
		...
		case MotionEvent.ACTION_UP:
			...
			if (Math.abs(x - downX) < itemClickMin) {// 这里根据点击距离来判断是否是itemClick
				if(activity instanceof AllWordActivity){
					...
					ReciteMyFavoriteActivity.ItemDeleteReset();
				}
				return false;
			}
			if (deltaX > 0) {
				if (scroll > back_width / 4) {
					...
					if(activity instanceof AllWordActivity){
						AllWordActivity.itemDelete = this;
					}else if (activity instanceof ReciteAlreadyKnowActivity){
						ReciteAlreadyKnowActivity.itemDelete = this;
					}else if (activity instanceof ReciteMyFavoriteActivity){
						ReciteMyFavoriteActivity.itemDelete = this;
					}
				} else {
					reSet();
				}
