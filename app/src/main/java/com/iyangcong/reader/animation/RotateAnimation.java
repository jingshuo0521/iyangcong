package com.iyangcong.reader.animation;
...
public class RotateAnimation extends Animation {
    ...
    public RotateAnimation(float fromDegree, float toDegree) {
    ...
    protected void applyTransformation(float interpolatedTime, Transformation t) {
        ...
        if (degree >= 82.0f) {
            ...
            //这里不是使用mCamera.rotateX(degree);因为由于没有mCamera.translate(0,0,-halfWidth);画面要旋转多少度我们看上去就是转了多少度（有了那句代码，画面旋转82度时我们看上去已经转了90度）
            //如果使用rotateX(degree),那么实际的效果就是，degree从0度到82度时，我们看到画面从0度转到了90度，画面消失，然后degree 83度时，画面又是旋转了83度的样子，并且是绕着自己的中轴线转的83度（没有
...
//			mCamera.translate(0, halfWidth, 0);
            ...
            //同理而言，前面那句mCamera.rotateX(degree);其实也不是真的让画面在三维世界里转动，其是绕Y轴转动效果只是图像x轴缩放与错切相结合产生的人眼幻觉。
            ...
            //为什么前面那句mCamera.translate(0, 0,halfWidth);没有产生x轴的平移呢？也许前面有平移，只是平移量是0.在rotateX之前，矩阵左下角那个值是0.0；随着degree的不同，左下角的值也不
            //同，也许这个值与translate的x轴平移量也有关系。
            //打印矩阵9个值，除了x轴平移外，本句代码还修改了scale_x。mCamera.translate(0, 0,halfWidth);产生的scale_x是一个定值，该值小于1；而mCamera.rotateX(degree);产生的scale_x是从定值到0.0。
