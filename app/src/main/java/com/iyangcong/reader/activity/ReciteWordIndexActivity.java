...
//        List<NewWord> newWordList = wordDao.all();
		initLevelNums();
//		List<NewWord> newWordList = wordDao.getWordListByIFreadyRecite(true);
		List<NewWord> newWordList = wordDao.getWordsByDeleteStatusAndIFreadyRecite();
		Logger.i("hahahahaha reciteWordList:" + newWordList);
		if (newWordList!=null&&newWordList.size()>0){
			for (NewWord newWord : newWordList) {
				...
				if (newWord.getCET4() == 0 && newWord.getCET6() == 0 && newWord.getGRE() == 0 && newWord.getIELTS() == 0 &&
						newWord.getSAT() == 0 && newWord.getTEM4() == 0 && newWord.getTEM8() == 0 && newWord.getTOEFL() == 0) {
					count_other++;
				}
			}
		}
...
//        }
	...
	private void getDatasFromNetwork() {
		OkGo.get(Urls.PersonReciteNewWordsDegreeInfoURL)
				.tag(this)
