...
bool ZLZipInputStream::open() {
	close();
    ZLLogger::Instance().registerClass("ZLZipInputStream");
    ZLLogger::Instance().println("ZLZipInputStream", "myBaseName=" + myBaseName);
    ZLLogger::Instance().println("ZLZipInputStream", "myEntryName=" + myEntryName);
...
std::size_t ZLZipInputStream::read(char *buffer, std::size_t maxSize) {
	...
	if (myIsDeflated) {
		...
		myOffset += realSize;
		ZLLogger::Instance().println("ZLZipInputStream", "isdeflated");
	} else {
	    ZLLogger::Instance().println("ZLZipInputStream", "notdeflated");
