...
void XHTMLTagParagraphAction::doAtStart(XHTMLReader &reader, const char**) {
	if (!reader.myNewParagraphInProgress) {
		...
		if (myTextKind==51) {
			reader.beginParagraph(false, ZLTextParagraph::Tag_P);
		} else {
			reader.beginParagraph(false, ZLTextParagraph::Tag_NOT_P);
		}
...
void XHTMLTagItemAction::doAtStart(XHTMLReader &reader, const char**) {
	...
	if (!reader.myListNumStack.empty()) {
		...
		if (index == 0) {
			static const std::string bullet = "\xE2\x80\xA2\xC0\xA0";//着重号
...
void XHTMLReader::fillTagTable() {
	if (ourTagActions.empty()) {
		...
		addAction("h1", new XHTMLTagControlAction(H1));
		addAction("h2", new XHTMLTagControlAction(H2));
		addAction("h3", new XHTMLTagControlAction(H3));
		addAction("h4", new XHTMLTagControlAction(H4));
		addAction("h5", new XHTMLTagControlAction(H5));
		addAction("h6", new XHTMLTagControlAction(H6));
...
//		addAction("h6", new XHTMLTagParagraphWithControlAction(H6));
        ...
        addAction("td",new XHTMLTagControlAction(TEST));
        addAction("th",new XHTMLTagControlAction(TEST));
...
bool XHTMLReader::readFile(const ZLFile &file, const std::string &referenceName) {
	fillTagTable();
    ZLLogger::Instance().registerClass("XHTMLReader");
	...
	myTableParser.reset();
	std::string str = file.physicalFilePath();
	ZLLogger::Instance().println("XHTMLReader", "physicalFilePath:" + str);
...
void XHTMLReader::beginParagraph(bool restarted,ZLTextParagraph::ParagraphTag tag) {
	myCurrentParagraphIsEmpty = true;
	myModelReader.beginParagraph(ZLTextParagraph::TEXT_PARAGRAPH,tag);
