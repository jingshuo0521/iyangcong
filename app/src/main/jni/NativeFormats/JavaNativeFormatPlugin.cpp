...
static jobject createTextModel(JNIEnv *env, jobject javaModel, ZLTextModel &model) {
    ...
    ZLLogger::Instance().println("asdfasf",model.language().c_str());
	...
	jbyteArray paragraphTags = env->NewByteArray(arraysSize);
	...
	env->SetByteArrayRegion(paragraphTags,0,arraysSize,&model.paragraphTags().front());
